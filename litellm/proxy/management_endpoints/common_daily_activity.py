...
def _has_request_activity(metrics: SpendMetrics) -> bool:
    """True when a rollup row carries per-request activity; PTU sentinel rows carry none."""
    return bool(metrics.api_requests or metrics.spend or metrics.total_tokens)
    ...
    *,
    ...
    for record in records:
        ...
        if level == _GROUP_DATE_API_KEY:
            ...
            # this level the sentinel's cost would land under "unknown". The provider
            # breakdown stays request-only, matching the per-row path, which skips
            # sentinel rows outright rather than attributing them to a provider.
            provider_metrics = metrics.model_copy(update={"flat_cost": 0.0})  # mutable-ok: pydantic update payload
            if _has_request_activity(provider_metrics):
                provider = record.custom_llm_provider or "unknown"
